...
// 宣告一個變數把爬到的所有資料給丟進去;
let webCrawler_totalContent = [];

async function loginFacebook() {
  // 建立這個Brower 的類型
  let driver = await new webdriver.Builder().forBrowser("chrome").withCapabilities(options).build();
  const web = "https://www.facebook.com/login/device-based/regular/login/?login_attempt=1&lwv=120&lwc=1348028"; //填寫要去的網站
  ...
  // 開始找尋文章貼文的div-class
  let fb_article_eles = await driver.findElements(By.className("x193iq5w xeuugli x13faqbe x1vvkbs xlh3980 xvmahel x1n0sxbx x1lliihq x1s928wv xhkezso x1gmr53x x1cpjm7i x1fgarty x1943h6x x4zkp8e x3x7a5m x6prxxf xvq8zen xo1l8bm xzsf02u x1yc453h"));

  //  fb_article_eles 確定是個陣列；
  // console.log(Array.isArray(fb_article_eles));\

  // 辨別所有"顯示更多"的位置並點擊一下;
  let clickSeemoreTimes = await driver.findElements(By.xpath(`//*[contains(@class,"xt0b8zv xzsf02u x1s688f") and text()="顯示更多"]`));
  // 再次確認是否有clickSeemoreTimes;
  // console.log(clickSeemoreTimes.length);
  for (let i = 0; i < clickSeemoreTimes.length; i++) {
    const fb_search_seemore = await driver.wait(until.elementLocated(By.xpath(`//*[contains(@class,"xt0b8zv xzsf02u x1s688f") and text()="顯示更多"]`)));
  ...
  for (const fb_article_ele of fb_article_eles) {
    ...
    if (fb_article_text.indexOf("賽事") > 0) {
      ...
      for (let x = 0; x < iterationTime; x++) {
        // 宣告一個陣列去儲存爬到的資料;
        let webCrawler_content = [];

        // 判斷是否有 xx/xx 格式的字串
        let fb_content_date = /\d{1,2}[/]\d{1,2}/g;
        let fb_content_dateMatch = await fb_content_date.exec(fb_content_v1)
        if (fb_content_dateMatch !== null) {
          console.log("date:" + fb_content_dateMatch[0])
          webCrawler_content.push("2023/" + fb_content_dateMatch[0]);
        } else {
          fb_content_dateMatch = "";
          webCrawler_content.push(fb_content_dateMatch);
          console.log("----沒有比賽日期----")
        };

        // 判斷是否有 xx:xx 時間格式字串
        let fb_content_time = /\d{1,2}:\d{1,2}/g;
        let fb_content_timeMatch = await fb_content_time.exec(fb_content_v2[x]);
        if (fb_content_timeMatch !== null) {
          console.log("time:" + fb_content_timeMatch[0]);
          webCrawler_content.push(fb_content_timeMatch[0]);
        } else {
          fb_content_timeMatch = "";
          webCrawler_content.push(fb_content_timeMatch);
          console.log("----沒有比賽時間----")
        };
        ...
        if (fb_content_nameMatch !== null) {
          ...
          webCrawler_content.push(fb_content_name);
        } else {
          let fb_content_name = "";
          webCrawler_content.push(fb_content_name);
          console.log("----沒有比賽名稱----");
        ...
        // 判斷是否有 報名費: XXX 的格式字串
        let fb_content_buyInMatch = await fb_content_v2[x].match(/報名費：(\d{1,4})/g);
        ...
        if (fb_content_buyInMatch !== null) {
          ...
          webCrawler_content.push(fb_content_buyIn);
        } else {
        ...
        let fb_content_gtdMatch = fb_content_v2[x].match(/保證獎池(\d{1,6}\S+)/g);
        ...
        if (fb_content_gtdMatch !== null) {
          ...
          webCrawler_content.push(fb_content_gtd);
        } else {
          let fb_content_gtd = "";
          webCrawler_content.push(fb_content_gtd);
          console.log("----沒有保證獎池----")
        ...
        let fb_content_gtdChampionMatch = await fb_content_v2[x].match(/冠軍\S+/g);
        ...
        if (fb_content_gtdChampionMatch !== null) {
          ...
          webCrawler_content.push(fb_content_gtdChampion.substring(2,));
        } else {
          let fb_content_gtdChampion = "";
          webCrawler_content.push(fb_content_gtdChampion);
          console.log("----沒有保證冠軍----");
        };
        ...
        if (fb_content_startingStackMatch !== null) {
          let fb_content_startingStack = fb_content_startingStackMatch[0].substring(4,);
          if (/\d+\.\d+[a-zA-Z]/.test(fb_content_startingStack)) {
            fb_content_startingStack = fb_content_startingStack.substring(0, 1) + fb_content_startingStack.substring(2, 3) + ",000";
          };
          console.log("startingStack:" + fb_content_startingStack);
          webCrawler_content.push(fb_content_startingStack);
        } else {
        ...
        let fb_content_levelTime = /升盲時間：(\d+min|\d+\/\d+分鐘|1小時)/g;
        let fb_content_levelTimeMatch = await fb_content_levelTime.exec(fb_content_v2[x])
        if (fb_content_levelTimeMatch !== null) {
          console.log("levelTime:" + fb_content_levelTimeMatch[1]); // 输出匹配到的升盲时间信息，例如：10min、20/15分鐘等
          webCrawler_content.push(fb_content_levelTimeMatch[1]);
        } else {
          let fb_content_levelTimeMatch = "";
          webCrawler_content.push(fb_content_levelTimeMatch);
          console.log("----沒有升盲時間----")
        };
        ...
        let fb_content_regClosedEndMatch = await fb_content_v2[x].match(/截止.*?\s\S+/);
        // console.log("全部的----regClosedEndMatch:"+fb_content_regClosedEndMatch);
        if (fb_content_regClosedEndMatch !== null) {
          let fb_content_regClosedEnd = fb_content_regClosedEndMatch[0];
          console.log("regClosedEnd:" + fb_content_regClosedEnd.substring(5,));
          webCrawler_content.push(fb_content_regClosedEnd.substring(5,));
        } else {
          let fb_content_regClosedEnd = "";
          webCrawler_content.push(fb_content_regClosedEnd);
          console.log("----沒有截止時間----");
        };
        webCrawler_totalContent.push(webCrawler_content);
        console.log("*********同篇貼文分隔線*********");
      }
    }
  }
  driver.quit();
  console.log(webCrawler_totalContent);
  /**
 ...
 */
  ...
  function writeData(auth) {
    ...
    let values = webCrawler_totalContent;
    let resource = {
      values,
    };
    // 檢查第一列是否有相同的資料
    ...
    sheets.spreadsheets.values.get({
      ...
      if (err) {
        ...
        console.log(err);
          ...
          let existingValues = result.data.values[0];
          let newValues = values[0];
          let match = existingValues.every((val, i) => val === newValues[i]);
